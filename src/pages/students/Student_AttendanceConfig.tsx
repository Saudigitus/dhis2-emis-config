...
function StudentsAttendance(): React.ReactElement {
  ...
  const getFormFields = () => {
    ...
    const foundProgramStage = getDataStoreElement({ dataStores: data.dataStoreConfigs, key: "student", elementKey: "attendance" })?.programStage
    const foundAbsenceReason = getDataStoreElement({ dataStores: data.dataStoreConfigs, key: "student", elementKey: "attendance" })?.absenceReason
    const foundStatus = getDataStoreElement({ dataStores: data.dataStoreConfigs, key: "student", elementKey: "attendance" })?.status
  ...
  const onSubmit = async (values: SubmitValue) => {
    try {
      if (values.programStage !== undefined && values.status !== undefined && values.absenceReason !== undefined) {
        ...
        const attendance = getDataStoreElement({ dataStores: data.dataStoreConfigs, key: "student", elementKey: "attendance" })
